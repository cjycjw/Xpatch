 ...
 * https://github.com/limpoxe/Android-Plugin-Framework/blob/master/FairyPlugin/src/main/java/com/limpoxe/fairy/util/PackageVerifyer.java
 */
...
//    }
     ...
     * @return
     */
    public static String collectCertificates(String sourcePath, boolean simpleMode) {
        ...
        try {
            ...
            if (simpleMode) {
                ...
                if (DEBUG) {
                    System.out.println("File " + sourcePath + ": entry=" + jarEntry
                            + " certs=" + (certs != null ? certs.length : 0));
                    if (certs != null) {
                        final int N = certs.length;
                        for (int i=0; i<N; i++) {
                ...
                while (entries.hasMoreElements()) {
                    ...
                    Certificate[] localCerts = loadCertificates(jarFile, je,
                            readBuffer);
                    if (DEBUG) {
                        System.out.println( "File " + sourcePath + " entry " + je.getName()
                                + ": certs=" + certs + " ("
                                + (certs != null ? certs.length : 0) + ")");
                    }
                    if (localCerts == null) {
                        ...
                        for (int i=0; i<certs.length; i++) {
                            ...
                            if (!found || certs.length != localCerts.length) {
                                ...
                                System.out.println("INSTALL_PARSE_FAILED_INCONSISTENT_CERTIFICATES");
                                return null;
                            }
                        }
                    }
            ...
            if (certs != null && certs.length > 0) {
                String signResult = new String(ApkSignatureHelper.toChars(certs[0].getEncoded()));
            ...
            System.out.println( "Exception reading " + sourcePath + ": " + e);
            System.out.println("INSTALL_PARSE_FAILED_CERTIFICATE_ENCODING");
            return null;
        } catch (IOException e) {
            System.out.println("Exception reading " + sourcePath + ": " + e);
            System.out.println("INSTALL_PARSE_FAILED_CERTIFICATE_ENCODING");
            ...
            System.out.println( "Exception reading " + sourcePath + ": " + e);
            System.out.println("INSTALL_PARSE_FAILED_UNEXPECTED_EXCEPTION");
            return null;
        }
